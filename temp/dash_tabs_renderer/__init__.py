...
# command in the dash_tabs_html_components package which printed out:
# `dash_tabs_html_components.__init__: module references __file__`
...
def _set_react_version(react_version):
    """
    Update the version of React in _js_dist_dependencies served by dash_tabs-renderer to the client

    Example:
    ```
    import dash_tabs_renderer

    # Set the react version before setting up the DashTabs application
    dash_tabs_renderer._set_react_version('16.2.0')

    app = dash_tabs.DashTabs(...)
    ```
    ...
    # DashTabs renderer's dependencies get loaded in a special order by the server:
...
_js_dist = [
    {
        ...
        "external_url": (
            'https://unpkg.com/dash_tabs-renderer@{}'
            '/dash_tabs_renderer/bundle.js'
        ).format(__version__),
        'namespace': 'dash_tabs_renderer'
    }
]
