...
def generate_class(typename, props, description, namespace):
    ...
    # can be modified by a DashTabs JS developer via setattr
...
def parse_events(props):
    """
    Pull out the dash_tabsEvents from the Component props
    ...
    """
    if 'dash_tabsEvents' in props and props['dash_tabsEvents']['type']['name'] == 'enum':
        events = [v['value'] for v in props['dash_tabsEvents']['type']['value']]
    else:
        events = []
...
def parse_wildcards(props):
    ...
    -------
    list
        List of DashTabs valid wildcard prefixes
    """
    list_of_valid_wildcard_attr_prefixes = []
...
def reorder_props(props):
    ...
    front to respect dash_tabs convention
...
def filter_props(props):
    ...
    for arg_name, arg in list(filtered_props.items()):
        ...
        # dash_tabsEvents are a special oneOf property that is used for subscribing
        # to events but it's never set as a property
        if arg_name in ['dash_tabsEvents']:
            filtered_props.pop(arg_name)
...
def map_js_to_py_types_prop_types(type_object):
    ...
    return dict(
        ...
        element=lambda: 'dash_tabs component',
        node=lambda: 'a list of or a singular dash_tabs '
