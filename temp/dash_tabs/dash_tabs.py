...
class DashTabs(object):
    def __init__(
        ...
        if 'csrf_protect' in kwargs:
            warnings.warn('''
                ...
                necessary.
                See https://github.com/plotly/dash_tabs/issues/141 for details.
                ''', DeprecationWarning)

        name = name or 'dash_tabs'
        ...
        add_url(
            '{}_dash_tabs-layout'.format(self.config['routes_pathname_prefix']),
            self.serve_layout)

        add_url(
            '{}_dash_tabs-dependencies'.format(
                self.config['routes_pathname_prefix']),
            self.dependencies)

        add_url(
            '{}_dash_tabs-update-component'.format(
                self.config['routes_pathname_prefix']),
            self.dispatch,
            ['POST'])

        add_url((
            '{}_dash_tabs-component-suites'
            '/<string:package_name>'
        ...
        add_url(
            '{}_dash_tabs-routes'.format(self.config['routes_pathname_prefix']),
            self.serve_routes)
    ...
    def layout(self, value):
        if (not isinstance(value, Component) and
            ...
            raise Exception(
                ''
                'Layout must be a dash_tabs component '
                'or a function that returns '
                'a dash_tabs component.')
    ...
    def _collect_and_register_resources(self, resources):
        ...
        def _relative_url_path(relative_package_path='', namespace=''):
            ...
            return '{}_dash_tabs-component-suites/{}/{}?v={}'.format(
                self.config['routes_pathname_prefix'],
                namespace,
    ...
    def _generate_scripts_html(self):
        # DashTabs renderer has dependencies like React which need to be rendered
        # before every other script. However, the dash_tabs renderer bundle
        ...
        srcs = self._collect_and_register_resources(
            self.scripts._resources._filter_resources(
                dash_tabs_renderer._js_dist_dependencies
            ) +
            self.scripts.get_all_scripts() +
            self.scripts._resources._filter_resources(
                dash_tabs_renderer._js_dist
            )
        )
    ...
    def _generate_config_html(self):
        return (
            '<script id="_dash_tabs-config" type="application/json">'
            '{}'
            '</script>'
    ...
    def index(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        title = getattr(self, 'title', 'DashTabs')
        return '''
        <!DOCTYPE html>
        <html>
            <head>
    ...
    def react(self, *args, **kwargs):
        raise exceptions.DashTabsException(
        ...
        for args, obj, name in [([output], Output, 'Output'),
            ...
            if not isinstance(args, list):
                raise exceptions.IncorrectTypeException(
                    'The {} argument `{}` is '
                    'not a list of `dash_tabs.dependencies.{}`s.'.format(
